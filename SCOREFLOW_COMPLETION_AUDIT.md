...
**P9 `[VERIFIED-by-mechanism]` Global pass counter never resets per section** (`parseMeasureRepeats.js:158,209-213`): after any expanded section, `pass` stays 2, so later repeat sections are treated as already on their final pass and later volta blocks select brackets with a stale pass number (compounds with P5/P6; visible in A-2/A-4 traces).
