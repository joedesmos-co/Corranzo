...
describe('Practice page simplification', () => {
  ...
  it('keeps the first Practice action path above stats and status', () => {
    ...
    const scopeIndex = panel.indexOf('<PracticeScopeSection')
    ...
    expect(scopeIndex).toBeGreaterThan(modeIndex)
    expect(waitForYouIndex).toBeGreaterThan(scopeIndex)
  ...
  it('uses beginner-facing Wait For You controls under Mode', () => {
    ...
    expect(waitForYou).not.toContain('wait-for-you__primary-action')
    expect(waitForYou).toContain('Pauses at each note in your loop until you play it or tap Continue.')
    expect(waitForYou).toContain('wait-for-you__now-playing')
    expect(waitForYou).toContain('wait-for-you__actions')
    expect(waitForYou).toContain('Continue')
    expect(practiceSession).toContain('practiceScopeAvailable')
