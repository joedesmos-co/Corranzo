...
const GENERATED = join(HERE, 'baseline-ledger/generated')
const DETAILED = JSON.parse(readFileSync(join(HERE, 'baseline-ledger/error_inventory.json'), 'utf8'))
const BASELINE = JSON.parse(readFileSync(join(HERE, 'baseline-original.json'), 'utf8'))
...
const ledger = enrichedRows.map((row) => {
  ...
  return {
    ...
    final_musicxml_provenance: {
      generated_file: `tmp/omr-zero-defect/baseline-ledger/generated/${row.fixture}.musicxml`,
    ...
    confidence,
    root_cause_cluster: audit ? 'benchmark-hidden-natural-cancellation' : cluster,
...
if (ledger.length !== 650 || !countsMatch) {
  throw new Error(`Ledger integrity mismatch: total=${ledger.length} byCode=${JSON.stringify(byCode)} expected=${JSON.stringify(expectedCounts)}`)
}
...
const payload = {
  ...
  current_head: '2366c3727d1c0794a57a2b1ef4ff185a7ce7ab29',
  generated_at: new Date().toISOString(),
  evaluator: { version: BASELINE.evaluatorVersion, schema: BASELINE.schemaVersion, unchanged: true },
  corpus_run: 'tmp/omr-zero-defect/baseline-original.json',
  summary: {
    ...
    unique_structural_incorrect_chord_events: HIGH_EXTREME.globalRegisterSummary.overall.incorrectChordCount,
