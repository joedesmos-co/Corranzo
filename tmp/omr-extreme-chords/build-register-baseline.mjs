 ...
 * Diagnostic only; writes under tmp/omr-extreme-chords/. Does not edit production.
 */
...
const OUT = join(ROOT, 'tmp/omr-extreme-chords')
...
function renderMarkdown(payload) {
  ...
  lines.push('')
  lines.push('- `chord_inventory.json` — full chord event records')
...
async function main() {
  ...
  writeFileSync(join(OUT, 'chord_inventory.json'), `${JSON.stringify(payload, null, 2)}\n`)
  writeFileSync(join(OUT, 'PHASE_1_REGISTER_BASELINE.md'), renderMarkdown(payload))
  process.stderr.write(
    `Wrote ${allChords.length} chord records → tmp/omr-extreme-chords/chord_inventory.json\n`,
  )
  process.stderr.write(
