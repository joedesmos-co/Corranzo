/**
 * Shadow rhythm solver benchmark comparison — runtime vs solver side-by-side.
...
import {
  ...
  summarizeRhythmShadowDiagnostics,
} from './scoreGraphRhythmSolver.js'
import {
  findMeasureEntry,
  measurePassesRhythmShadowTruthGate,
} from './omrRhythmShadowMeasureGate.js'

export function flattenMeasuresFromPages(pages = []) {
  const measures = []
  ...
  return measures.sort((left, right) => (left.measureNumber ?? 0) - (right.measureNumber ?? 0))
}
...
/**
 * Build runtime vs rhythm-shadow comparison when ScoreGraph IR + truth are available.
 */
export function buildRhythmShadowBenchmarkComparison({
  report = {},
  fixtureId = null,
  ...
  if (!resolvedGraph?.measures?.length) {
    return {
      ...
      reason:
        'ScoreGraph IR measures missing from report — re-run live dashboard with includeScoreGraph or pass scoreGraphFull.',
  ...
  const rhythmSolved = solveScoreGraphRhythmShadow(resolvedGraph)
  const structurallyApplied = rhythmSolved.measures.filter((measure) => measure.applied)
  ...
  const shadowXml =
    appliedMeasureCount > 0
      ? buildOmrMusicXml({
          title: `${title ?? fixtureId ?? 'shadow'}-rhythm-shadow`,
  ...
  const solverDiagnostics = summarizeRhythmShadowDiagnostics(rhythmSolved.measures)
  ...
  const rejectedMeasures = rhythmSolved.measures
  ...
  const base = {
    engine: 'v2-rhythm-shadow-prototype',
    constraintVersion: solverDiagnostics.constraintVersion ?? 'phase-2c',
    promoted: false,
    fixtureId,
    solverDiagnostics,
  ...
  const regressionReason =
    status === 'diagnostic-only-regressed'
      ? 'Shadow rhythm solver regressed chord and/or duration vs runtime — kept diagnostic-only (no promotion).'
      : status === 'diagnostic-only-mixed'
        ? 'Shadow solver mixed wins/losses across axes — not ready for promotion.'
        : status === 'shadow-no-qualifying-measures'
          ? 'No measures passed Phase 2C preservation + truth gates — shadow identical to runtime.'
          : null
...
export function formatRhythmShadowMarkdown(comparison) {
  if (!comparison || comparison.status === 'unavailable') {
    return ''
  }
  const lines = ['', '### Rhythm shadow solver (V2 Phase 2 — diagnostic only)', '']
  ...
  lines.push(
    `- Solver: ${comparison.solverDiagnostics?.changedMeasures ?? 0} accepted / ${comparison.solverDiagnostics?.candidateMeasures ?? 0} candidates / ${comparison.rejectedCandidateMeasures?.length ?? comparison.solverDiagnostics?.rejectedMeasures ?? 0} rejected`,
  )
  if (comparison.coalescedChordCount) {
    lines.push(`- Coalesced chords: ${comparison.coalescedChordCount}`)
  }
  if (comparison.structurallyAppliedCount != null && comparison.structurallyAppliedCount !== comparison.solverDiagnostics?.changedMeasures) {
    lines.push(
      `- Structural candidates: ${comparison.structurallyAppliedCount}; truth-approved: ${comparison.solverDiagnostics?.changedMeasures ?? 0}`,
    )
  }
  ...
  if (comparison.coalescedChords?.length) {
    const preview = comparison.coalescedChords
      .slice(0, 6)
      .map((entry) => `m${entry.measureNumber}:${entry.coalescedChords?.length ?? 0}`)
      .join(', ')
    lines.push(`- Coalesced chord events: ${preview}${comparison.coalescedChords.length > 6 ? '…' : ''}`)
  }
  ...
  } else if (comparison.changedMeasures?.length) {
    const preview = comparison.changedMeasures
      .slice(0, 12)
      .map((entry) => `m${entry.measureNumber}:${entry.variantId}`)
      .join(', ')
    lines.push(`- Changed measures: ${preview}${comparison.changedMeasures.length > 12 ? '…' : ''}`)
  }
  if (comparison.rejectedCandidateMeasures?.length) {
    const preview = comparison.rejectedCandidateMeasures
      .slice(0, 8)
      .map((entry) => {
        const variantReject = entry.rejectedVariants?.[0]
        const detail = variantReject?.violations?.join('+') ?? entry.rejectReason ?? 'rejected'
        return `m${entry.measureNumber}:${detail}`
      })
      .join(', ')
    lines.push(
      `- Rejected candidates: ${preview}${comparison.rejectedCandidateMeasures.length > 8 ? '…' : ''}`,
    )
  }
  if (comparison.hotspotDelta?.length) {
    lines.push('- Hotspot per-measure Δ:')
    for (const entry of comparison.hotspotDelta) {
      lines.push(
        `  - m${entry.measureNumber}: onset Δ ${entry.wrongOnsetDelta}, duration Δ ${entry.wrongDurationDelta}, chord Δ ${entry.chordMismatchDelta}`,
      )
    }
  }
  lines.push('')
